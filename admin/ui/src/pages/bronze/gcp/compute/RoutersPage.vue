...
const ENDPOINT = '/api/v1/bronze/gcp/compute/routers'

const columns: ColumnDef[] = [
  { key: 'name', format: 'bold' },
  { key: 'region', transform: shortPath },
  { key: 'network', transform: shortPath },
  { key: 'bgp_asn', label: 'BGP ASN', format: 'mono' },
  { key: 'bgp_advertise_mode', label: 'Advertise Mode' },
...
const drawerFields: DrawerFieldDef[] = [
  ...
  { key: 'bgp_advertise_mode', label: 'BGP Advertise Mode' },
  { key: 'bgp_keepalive_interval', label: 'BGP Keepalive Interval', transform: (v) => v ? `${v}s` : '' },
  { key: 'encrypted_interconnect_router', label: 'Encrypted Interconnect', format: 'bool' },
...
<template>
  <HTablePage
    ...
    export-filename="gcp-compute-routers.csv"
    title="Routers"
  />
</template>
