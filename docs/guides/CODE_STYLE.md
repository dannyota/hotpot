...
Never expose internal error details to clients. Use `WriteServerError` for all 500s:

```go
// Good — logs error server-side, generic message to client
admin.WriteServerError(w, "failed to load instances", err)

// Bad — leaks DB details, hostnames, driver errors
admin.WriteError(w, http.StatusInternalServerError, "count: "+err.Error())
```

`WriteError` with 400 status is fine for validation (user input errors).
