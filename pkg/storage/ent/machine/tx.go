...
func (tx *Tx) init() {
	tx.SilverMachine = NewSilverMachineClient(tx.config)
	tx.SilverMachineBronzeLink = NewSilverMachineBronzeLinkClient(tx.config)
	tx.SilverMachineNormalized = NewSilverMachineNormalizedClient(tx.config)
}
...
// applies a query, for example: SilverMachine.QueryXXX(), the query will be executed
