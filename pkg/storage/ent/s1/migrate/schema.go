// Code generated by ent, DO NOT EDIT.

package migrate

import (
	"entgo.io/ent/dialect/entsql"
	"entgo.io/ent/dialect/sql/schema"
	"entgo.io/ent/schema/field"
)

var (
	// S1AccountsHistoryColumns holds the columns for the "s1_accounts_history" table.
	S1AccountsHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "name", Type: field.TypeString},
		{Name: "state", Type: field.TypeString, Nullable: true},
		{Name: "account_type", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "expiration", Type: field.TypeTime, Nullable: true},
		{Name: "unlimited_expiration", Type: field.TypeBool, Default: false},
		{Name: "active_agents", Type: field.TypeInt, Default: 0},
		{Name: "total_licenses", Type: field.TypeInt, Default: 0},
		{Name: "usage_type", Type: field.TypeString, Nullable: true},
		{Name: "billing_mode", Type: field.TypeString, Nullable: true},
		{Name: "creator", Type: field.TypeString, Nullable: true},
		{Name: "creator_id", Type: field.TypeString, Nullable: true},
		{Name: "number_of_sites", Type: field.TypeInt, Default: 0},
		{Name: "external_id", Type: field.TypeString, Nullable: true},
		{Name: "licenses_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1AccountsHistoryTable holds the schema information for the "s1_accounts_history" table.
	S1AccountsHistoryTable = &schema.Table{
		Name:       "s1_accounts_history",
		Columns:    S1AccountsHistoryColumns,
		PrimaryKey: []*schema.Column{S1AccountsHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1account_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1AccountsHistoryColumns[5], S1AccountsHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1account_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1AccountsHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1account_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1AccountsHistoryColumns[3]},
			},
		},
	}
	// S1AgentsHistoryColumns holds the columns for the "s1_agents_history" table.
	S1AgentsHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "computer_name", Type: field.TypeString},
		{Name: "external_ip", Type: field.TypeString, Nullable: true},
		{Name: "site_name", Type: field.TypeString, Nullable: true},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "account_name", Type: field.TypeString, Nullable: true},
		{Name: "agent_version", Type: field.TypeString, Nullable: true},
		{Name: "os_type", Type: field.TypeString, Nullable: true},
		{Name: "os_name", Type: field.TypeString, Nullable: true},
		{Name: "os_revision", Type: field.TypeString, Nullable: true},
		{Name: "os_arch", Type: field.TypeString, Nullable: true},
		{Name: "is_active", Type: field.TypeBool, Default: false},
		{Name: "is_infected", Type: field.TypeBool, Default: false},
		{Name: "is_decommissioned", Type: field.TypeBool, Default: false},
		{Name: "machine_type", Type: field.TypeString, Nullable: true},
		{Name: "domain", Type: field.TypeString, Nullable: true},
		{Name: "uuid", Type: field.TypeString, Nullable: true},
		{Name: "network_status", Type: field.TypeString, Nullable: true},
		{Name: "last_active_date", Type: field.TypeTime, Nullable: true},
		{Name: "registered_at", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "os_start_time", Type: field.TypeTime, Nullable: true},
		{Name: "active_threats", Type: field.TypeInt, Default: 0},
		{Name: "encrypted_applications", Type: field.TypeBool, Default: false},
		{Name: "group_name", Type: field.TypeString, Nullable: true},
		{Name: "group_id", Type: field.TypeString, Nullable: true},
		{Name: "cpu_count", Type: field.TypeInt, Default: 0},
		{Name: "core_count", Type: field.TypeInt, Default: 0},
		{Name: "cpu_id", Type: field.TypeString, Nullable: true},
		{Name: "total_memory", Type: field.TypeInt64, Default: 0},
		{Name: "model_name", Type: field.TypeString, Nullable: true},
		{Name: "serial_number", Type: field.TypeString, Nullable: true},
		{Name: "storage_encryption_status", Type: field.TypeString, Nullable: true},
		{Name: "network_interfaces_json", Type: field.TypeJSON, Nullable: true},
		{Name: "site_id", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "os_username", Type: field.TypeString, Nullable: true},
		{Name: "group_ip", Type: field.TypeString, Nullable: true},
		{Name: "scan_status", Type: field.TypeString, Nullable: true},
		{Name: "scan_started_at", Type: field.TypeTime, Nullable: true},
		{Name: "scan_finished_at", Type: field.TypeTime, Nullable: true},
		{Name: "mitigation_mode", Type: field.TypeString, Nullable: true},
		{Name: "mitigation_mode_suspicious", Type: field.TypeString, Nullable: true},
		{Name: "last_logged_in_user_name", Type: field.TypeString, Nullable: true},
		{Name: "installer_type", Type: field.TypeString, Nullable: true},
		{Name: "external_id", Type: field.TypeString, Nullable: true},
		{Name: "last_ip_to_mgmt", Type: field.TypeString, Nullable: true},
		{Name: "is_up_to_date", Type: field.TypeBool, Default: false},
		{Name: "is_pending_uninstall", Type: field.TypeBool, Default: false},
		{Name: "is_uninstalled", Type: field.TypeBool, Default: false},
		{Name: "apps_vulnerability_status", Type: field.TypeString, Nullable: true},
		{Name: "console_migration_status", Type: field.TypeString, Nullable: true},
		{Name: "ranger_version", Type: field.TypeString, Nullable: true},
		{Name: "ranger_status", Type: field.TypeString, Nullable: true},
		{Name: "active_directory_json", Type: field.TypeJSON, Nullable: true},
		{Name: "locations_json", Type: field.TypeJSON, Nullable: true},
		{Name: "user_actions_needed_json", Type: field.TypeJSON, Nullable: true},
		{Name: "missing_permissions_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1AgentsHistoryTable holds the schema information for the "s1_agents_history" table.
	S1AgentsHistoryTable = &schema.Table{
		Name:       "s1_agents_history",
		Columns:    S1AgentsHistoryColumns,
		PrimaryKey: []*schema.Column{S1AgentsHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1agent_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsHistoryColumns[5], S1AgentsHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1agent_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1agent_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1agent_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsHistoryColumns[9]},
			},
		},
	}
	// S1AgentNicsHistoryColumns holds the columns for the "s1_agent_nics_history" table.
	S1AgentNicsHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "agent_history_id", Type: field.TypeUint},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "interface_id", Type: field.TypeString, Nullable: true},
		{Name: "name", Type: field.TypeString, Nullable: true},
		{Name: "description", Type: field.TypeString, Nullable: true},
		{Name: "type", Type: field.TypeString, Nullable: true},
		{Name: "inet_json", Type: field.TypeJSON, Nullable: true},
		{Name: "inet6_json", Type: field.TypeJSON, Nullable: true},
		{Name: "physical", Type: field.TypeString, Nullable: true},
		{Name: "gateway_ip", Type: field.TypeString, Nullable: true},
		{Name: "gateway_mac", Type: field.TypeString, Nullable: true},
	}
	// S1AgentNicsHistoryTable holds the schema information for the "s1_agent_nics_history" table.
	S1AgentNicsHistoryTable = &schema.Table{
		Name:       "s1_agent_nics_history",
		Columns:    S1AgentNicsHistoryColumns,
		PrimaryKey: []*schema.Column{S1AgentNicsHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1agentnic_agent_history_id",
				Unique:  false,
				Columns: []*schema.Column{S1AgentNicsHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1agentnic_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1AgentNicsHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1agentnic_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1AgentNicsHistoryColumns[3]},
			},
		},
	}
	// S1AppInventoryHistoryColumns holds the columns for the "s1_app_inventory_history" table.
	S1AppInventoryHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "application_name", Type: field.TypeString},
		{Name: "application_vendor", Type: field.TypeString, Nullable: true},
		{Name: "endpoints_count", Type: field.TypeInt, Nullable: true},
		{Name: "application_versions_count", Type: field.TypeInt, Nullable: true},
		{Name: "estimate", Type: field.TypeBool, Default: false},
	}
	// S1AppInventoryHistoryTable holds the schema information for the "s1_app_inventory_history" table.
	S1AppInventoryHistoryTable = &schema.Table{
		Name:       "s1_app_inventory_history",
		Columns:    S1AppInventoryHistoryColumns,
		PrimaryKey: []*schema.Column{S1AppInventoryHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1appinventory_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1AppInventoryHistoryColumns[5], S1AppInventoryHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1appinventory_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1AppInventoryHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1appinventory_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1AppInventoryHistoryColumns[3]},
			},
		},
	}
	// S1EndpointAppsHistoryColumns holds the columns for the "s1_endpoint_apps_history" table.
	S1EndpointAppsHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "agent_id", Type: field.TypeString},
		{Name: "name", Type: field.TypeString},
		{Name: "version", Type: field.TypeString, Nullable: true},
		{Name: "publisher", Type: field.TypeString, Nullable: true},
		{Name: "size", Type: field.TypeInt, Nullable: true},
		{Name: "installed_date", Type: field.TypeTime, Nullable: true},
	}
	// S1EndpointAppsHistoryTable holds the schema information for the "s1_endpoint_apps_history" table.
	S1EndpointAppsHistoryTable = &schema.Table{
		Name:       "s1_endpoint_apps_history",
		Columns:    S1EndpointAppsHistoryColumns,
		PrimaryKey: []*schema.Column{S1EndpointAppsHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1endpointapp_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsHistoryColumns[5], S1EndpointAppsHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1endpointapp_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1endpointapp_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1endpointapp_agent_id",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsHistoryColumns[6]},
			},
		},
	}
	// S1GroupsHistoryColumns holds the columns for the "s1_groups_history" table.
	S1GroupsHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "name", Type: field.TypeString},
		{Name: "site_id", Type: field.TypeString, Nullable: true},
		{Name: "type", Type: field.TypeString, Nullable: true},
		{Name: "is_default", Type: field.TypeBool, Default: false},
		{Name: "inherits", Type: field.TypeBool, Default: false},
		{Name: "rank", Type: field.TypeInt, Nullable: true},
		{Name: "total_agents", Type: field.TypeInt, Default: 0},
		{Name: "creator", Type: field.TypeString, Nullable: true},
		{Name: "creator_id", Type: field.TypeString, Nullable: true},
		{Name: "filter_name", Type: field.TypeString, Nullable: true},
		{Name: "filter_id", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "registration_token", Type: field.TypeString, Nullable: true},
	}
	// S1GroupsHistoryTable holds the schema information for the "s1_groups_history" table.
	S1GroupsHistoryTable = &schema.Table{
		Name:       "s1_groups_history",
		Columns:    S1GroupsHistoryColumns,
		PrimaryKey: []*schema.Column{S1GroupsHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1group_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1GroupsHistoryColumns[5], S1GroupsHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1group_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1GroupsHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1group_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1GroupsHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1group_site_id",
				Unique:  false,
				Columns: []*schema.Column{S1GroupsHistoryColumns[7]},
			},
		},
	}
	// S1NetworkDiscoveriesHistoryColumns holds the columns for the "s1_network_discoveries_history" table.
	S1NetworkDiscoveriesHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "name", Type: field.TypeString, Nullable: true},
		{Name: "ip_address", Type: field.TypeString, Nullable: true},
		{Name: "domain", Type: field.TypeString, Nullable: true},
		{Name: "serial_number", Type: field.TypeString, Nullable: true},
		{Name: "category", Type: field.TypeString, Nullable: true},
		{Name: "sub_category", Type: field.TypeString, Nullable: true},
		{Name: "resource_type", Type: field.TypeString, Nullable: true},
		{Name: "os", Type: field.TypeString, Nullable: true},
		{Name: "os_family", Type: field.TypeString, Nullable: true},
		{Name: "os_version", Type: field.TypeString, Nullable: true},
		{Name: "os_name_version", Type: field.TypeString, Nullable: true},
		{Name: "architecture", Type: field.TypeString, Nullable: true},
		{Name: "manufacturer", Type: field.TypeString, Nullable: true},
		{Name: "cpu", Type: field.TypeString, Nullable: true},
		{Name: "memory_readable", Type: field.TypeString, Nullable: true},
		{Name: "network_name", Type: field.TypeString, Nullable: true},
		{Name: "asset_status", Type: field.TypeString, Nullable: true},
		{Name: "asset_criticality", Type: field.TypeString, Nullable: true},
		{Name: "asset_environment", Type: field.TypeString, Nullable: true},
		{Name: "infection_status", Type: field.TypeString, Nullable: true},
		{Name: "device_review", Type: field.TypeString, Nullable: true},
		{Name: "epp_unsupported_unknown", Type: field.TypeString, Nullable: true},
		{Name: "asset_contact_email", Type: field.TypeString, Nullable: true},
		{Name: "legacy_identity_policy_name", Type: field.TypeString, Nullable: true},
		{Name: "previous_os_type", Type: field.TypeString, Nullable: true},
		{Name: "previous_os_version", Type: field.TypeString, Nullable: true},
		{Name: "previous_device_function", Type: field.TypeString, Nullable: true},
		{Name: "detected_from_site", Type: field.TypeString, Nullable: true},
		{Name: "s1_account_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_account_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_site_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_site_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_group_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_group_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_scope_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_scope_level", Type: field.TypeString, Nullable: true},
		{Name: "s1_scope_path", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_account_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_group_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_site_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_scope_level", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_scope_path", Type: field.TypeString, Nullable: true},
		{Name: "memory", Type: field.TypeInt, Nullable: true},
		{Name: "core_count", Type: field.TypeInt, Nullable: true},
		{Name: "s1_management_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_scope_type", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_account_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_group_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_scope_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_site_id", Type: field.TypeInt, Nullable: true},
		{Name: "is_ad_connector", Type: field.TypeBool, Default: false},
		{Name: "is_dc_server", Type: field.TypeBool, Default: false},
		{Name: "ads_enabled", Type: field.TypeBool, Default: false},
		{Name: "first_seen_dt", Type: field.TypeTime, Nullable: true},
		{Name: "last_update_dt", Type: field.TypeTime, Nullable: true},
		{Name: "last_active_dt", Type: field.TypeTime, Nullable: true},
		{Name: "last_reboot_dt", Type: field.TypeTime, Nullable: true},
		{Name: "s1_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "agent_json", Type: field.TypeJSON, Nullable: true},
		{Name: "network_interfaces_json", Type: field.TypeJSON, Nullable: true},
		{Name: "alerts_json", Type: field.TypeJSON, Nullable: true},
		{Name: "alerts_count_json", Type: field.TypeJSON, Nullable: true},
		{Name: "device_review_log_json", Type: field.TypeJSON, Nullable: true},
		{Name: "identity_json", Type: field.TypeJSON, Nullable: true},
		{Name: "notes_json", Type: field.TypeJSON, Nullable: true},
		{Name: "tags_json", Type: field.TypeJSON, Nullable: true},
		{Name: "missing_coverage_json", Type: field.TypeJSON, Nullable: true},
		{Name: "subnets_json", Type: field.TypeJSON, Nullable: true},
		{Name: "surfaces_json", Type: field.TypeJSON, Nullable: true},
		{Name: "network_names_json", Type: field.TypeJSON, Nullable: true},
		{Name: "risk_factors_json", Type: field.TypeJSON, Nullable: true},
		{Name: "active_coverage_json", Type: field.TypeJSON, Nullable: true},
		{Name: "discovery_methods_json", Type: field.TypeJSON, Nullable: true},
		{Name: "hostnames_json", Type: field.TypeJSON, Nullable: true},
		{Name: "internal_ips_json", Type: field.TypeJSON, Nullable: true},
		{Name: "internal_ips_v6_json", Type: field.TypeJSON, Nullable: true},
		{Name: "mac_addresses_json", Type: field.TypeJSON, Nullable: true},
		{Name: "gateway_ips_json", Type: field.TypeJSON, Nullable: true},
		{Name: "gateway_macs_json", Type: field.TypeJSON, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "ranger_tags_json", Type: field.TypeJSON, Nullable: true},
		{Name: "id_secondary_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1NetworkDiscoveriesHistoryTable holds the schema information for the "s1_network_discoveries_history" table.
	S1NetworkDiscoveriesHistoryTable = &schema.Table{
		Name:       "s1_network_discoveries_history",
		Columns:    S1NetworkDiscoveriesHistoryColumns,
		PrimaryKey: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1networkdiscovery_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[5], S1NetworkDiscoveriesHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1networkdiscovery_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1networkdiscovery_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1networkdiscovery_asset_status",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[22]},
			},
			{
				Name:    "bronzehistorys1networkdiscovery_s1_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[34]},
			},
			{
				Name:    "bronzehistorys1networkdiscovery_s1_site_id",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[36]},
			},
			{
				Name:    "bronzehistorys1networkdiscovery_infection_status",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesHistoryColumns[25]},
			},
		},
	}
	// S1RangerDevicesHistoryColumns holds the columns for the "s1_ranger_devices_history" table.
	S1RangerDevicesHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "local_ip", Type: field.TypeString, Nullable: true},
		{Name: "external_ip", Type: field.TypeString, Nullable: true},
		{Name: "mac_address", Type: field.TypeString, Nullable: true},
		{Name: "os_type", Type: field.TypeString, Nullable: true},
		{Name: "os_name", Type: field.TypeString, Nullable: true},
		{Name: "os_version", Type: field.TypeString, Nullable: true},
		{Name: "device_type", Type: field.TypeString, Nullable: true},
		{Name: "device_function", Type: field.TypeString, Nullable: true},
		{Name: "manufacturer", Type: field.TypeString, Nullable: true},
		{Name: "managed_state", Type: field.TypeString, Nullable: true},
		{Name: "agent_id", Type: field.TypeString, Nullable: true},
		{Name: "first_seen", Type: field.TypeTime, Nullable: true},
		{Name: "last_seen", Type: field.TypeTime, Nullable: true},
		{Name: "subnet_address", Type: field.TypeString, Nullable: true},
		{Name: "gateway_ip_address", Type: field.TypeString, Nullable: true},
		{Name: "gateway_mac_address", Type: field.TypeString, Nullable: true},
		{Name: "network_name", Type: field.TypeString, Nullable: true},
		{Name: "domain", Type: field.TypeString, Nullable: true},
		{Name: "site_name", Type: field.TypeString, Nullable: true},
		{Name: "device_review", Type: field.TypeString, Nullable: true},
		{Name: "has_identity", Type: field.TypeBool, Default: false},
		{Name: "has_user_label", Type: field.TypeBool, Default: false},
		{Name: "fingerprint_score", Type: field.TypeInt, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "hostnames_json", Type: field.TypeJSON, Nullable: true},
		{Name: "discovery_methods_json", Type: field.TypeJSON, Nullable: true},
		{Name: "networks_json", Type: field.TypeJSON, Nullable: true},
		{Name: "tags_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1RangerDevicesHistoryTable holds the schema information for the "s1_ranger_devices_history" table.
	S1RangerDevicesHistoryTable = &schema.Table{
		Name:       "s1_ranger_devices_history",
		Columns:    S1RangerDevicesHistoryColumns,
		PrimaryKey: []*schema.Column{S1RangerDevicesHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1rangerdevice_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesHistoryColumns[5], S1RangerDevicesHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1rangerdevice_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1rangerdevice_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1rangerdevice_managed_state",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesHistoryColumns[15]},
			},
			{
				Name:    "bronzehistorys1rangerdevice_agent_id",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesHistoryColumns[16]},
			},
		},
	}
	// S1RangerGatewaysHistoryColumns holds the columns for the "s1_ranger_gateways_history" table.
	S1RangerGatewaysHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "ip", Type: field.TypeString, Nullable: true},
		{Name: "mac_address", Type: field.TypeString, Nullable: true},
		{Name: "external_ip", Type: field.TypeString, Nullable: true},
		{Name: "manufacturer", Type: field.TypeString, Nullable: true},
		{Name: "network_name", Type: field.TypeString, Nullable: true},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "account_name", Type: field.TypeString, Nullable: true},
		{Name: "site_id", Type: field.TypeString, Nullable: true},
		{Name: "number_of_agents", Type: field.TypeInt, Nullable: true},
		{Name: "number_of_rangers", Type: field.TypeInt, Nullable: true},
		{Name: "connected_rangers", Type: field.TypeInt, Nullable: true},
		{Name: "total_agents", Type: field.TypeInt, Nullable: true},
		{Name: "agent_percentage", Type: field.TypeFloat64, Nullable: true},
		{Name: "allow_scan", Type: field.TypeBool, Default: false},
		{Name: "archived", Type: field.TypeBool, Default: false},
		{Name: "new_network", Type: field.TypeBool, Default: false},
		{Name: "inherit_settings", Type: field.TypeBool, Default: false},
		{Name: "tcp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "udp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "icmp_scan", Type: field.TypeBool, Default: false},
		{Name: "smb_scan", Type: field.TypeBool, Default: false},
		{Name: "mdns_scan", Type: field.TypeBool, Default: false},
		{Name: "rdns_scan", Type: field.TypeBool, Default: false},
		{Name: "snmp_scan", Type: field.TypeBool, Default: false},
		{Name: "scan_only_local_subnets", Type: field.TypeBool, Default: false},
		{Name: "created_at_api", Type: field.TypeTime, Nullable: true},
		{Name: "expiry_date", Type: field.TypeTime, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "restrictions_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1RangerGatewaysHistoryTable holds the schema information for the "s1_ranger_gateways_history" table.
	S1RangerGatewaysHistoryTable = &schema.Table{
		Name:       "s1_ranger_gateways_history",
		Columns:    S1RangerGatewaysHistoryColumns,
		PrimaryKey: []*schema.Column{S1RangerGatewaysHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1rangergateway_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysHistoryColumns[5], S1RangerGatewaysHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1rangergateway_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1rangergateway_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1rangergateway_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysHistoryColumns[11]},
			},
		},
	}
	// S1RangerSettingsHistoryColumns holds the columns for the "s1_ranger_settings_history" table.
	S1RangerSettingsHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "scope_id", Type: field.TypeString, Nullable: true},
		{Name: "enabled", Type: field.TypeBool, Default: false},
		{Name: "use_periodic_snapshots", Type: field.TypeBool, Default: false},
		{Name: "snapshot_period", Type: field.TypeInt, Nullable: true},
		{Name: "network_decommission_value", Type: field.TypeInt, Nullable: true},
		{Name: "min_agents_in_network_to_scan", Type: field.TypeInt, Nullable: true},
		{Name: "tcp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "udp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "icmp_scan", Type: field.TypeBool, Default: false},
		{Name: "smb_scan", Type: field.TypeBool, Default: false},
		{Name: "mdns_scan", Type: field.TypeBool, Default: false},
		{Name: "rdns_scan", Type: field.TypeBool, Default: false},
		{Name: "snmp_scan", Type: field.TypeBool, Default: false},
		{Name: "multi_scan_ssdp", Type: field.TypeBool, Default: false},
		{Name: "use_full_dns_scan", Type: field.TypeBool, Default: false},
		{Name: "scan_only_local_subnets", Type: field.TypeBool, Default: false},
		{Name: "auto_enable_networks", Type: field.TypeBool, Default: false},
		{Name: "combine_devices", Type: field.TypeBool, Default: false},
		{Name: "new_network_in_hours", Type: field.TypeInt, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "restrictions_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1RangerSettingsHistoryTable holds the schema information for the "s1_ranger_settings_history" table.
	S1RangerSettingsHistoryTable = &schema.Table{
		Name:       "s1_ranger_settings_history",
		Columns:    S1RangerSettingsHistoryColumns,
		PrimaryKey: []*schema.Column{S1RangerSettingsHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1rangersetting_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1RangerSettingsHistoryColumns[5], S1RangerSettingsHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1rangersetting_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1RangerSettingsHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1rangersetting_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1RangerSettingsHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1rangersetting_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1RangerSettingsHistoryColumns[6]},
			},
		},
	}
	// S1SitesHistoryColumns holds the columns for the "s1_sites_history" table.
	S1SitesHistoryColumns = []*schema.Column{
		{Name: "history_id", Type: field.TypeUint, Increment: true},
		{Name: "valid_from", Type: field.TypeTime},
		{Name: "valid_to", Type: field.TypeTime, Nullable: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "resource_id", Type: field.TypeString},
		{Name: "name", Type: field.TypeString},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "account_name", Type: field.TypeString, Nullable: true},
		{Name: "state", Type: field.TypeString, Nullable: true},
		{Name: "site_type", Type: field.TypeString, Nullable: true},
		{Name: "suite", Type: field.TypeString, Nullable: true},
		{Name: "creator", Type: field.TypeString, Nullable: true},
		{Name: "creator_id", Type: field.TypeString, Nullable: true},
		{Name: "health_status", Type: field.TypeBool, Default: false},
		{Name: "active_licenses", Type: field.TypeInt, Default: 0},
		{Name: "total_licenses", Type: field.TypeInt, Default: 0},
		{Name: "unlimited_licenses", Type: field.TypeBool, Default: false},
		{Name: "is_default", Type: field.TypeBool, Default: false},
		{Name: "description", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "expiration", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "external_id", Type: field.TypeString, Nullable: true},
		{Name: "sku", Type: field.TypeString, Nullable: true},
		{Name: "usage_type", Type: field.TypeString, Nullable: true},
		{Name: "unlimited_expiration", Type: field.TypeBool, Default: false},
		{Name: "inherit_account_expiration", Type: field.TypeBool, Default: false},
		{Name: "licenses_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1SitesHistoryTable holds the schema information for the "s1_sites_history" table.
	S1SitesHistoryTable = &schema.Table{
		Name:       "s1_sites_history",
		Columns:    S1SitesHistoryColumns,
		PrimaryKey: []*schema.Column{S1SitesHistoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzehistorys1site_resource_id_valid_from",
				Unique:  false,
				Columns: []*schema.Column{S1SitesHistoryColumns[5], S1SitesHistoryColumns[1]},
			},
			{
				Name:    "bronzehistorys1site_valid_to",
				Unique:  false,
				Columns: []*schema.Column{S1SitesHistoryColumns[2]},
			},
			{
				Name:    "bronzehistorys1site_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1SitesHistoryColumns[3]},
			},
			{
				Name:    "bronzehistorys1site_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1SitesHistoryColumns[7]},
			},
		},
	}
	// S1AccountsColumns holds the columns for the "s1_accounts" table.
	S1AccountsColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "name", Type: field.TypeString},
		{Name: "state", Type: field.TypeString, Nullable: true},
		{Name: "account_type", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "expiration", Type: field.TypeTime, Nullable: true},
		{Name: "unlimited_expiration", Type: field.TypeBool, Default: false},
		{Name: "active_agents", Type: field.TypeInt, Default: 0},
		{Name: "total_licenses", Type: field.TypeInt, Default: 0},
		{Name: "usage_type", Type: field.TypeString, Nullable: true},
		{Name: "billing_mode", Type: field.TypeString, Nullable: true},
		{Name: "creator", Type: field.TypeString, Nullable: true},
		{Name: "creator_id", Type: field.TypeString, Nullable: true},
		{Name: "number_of_sites", Type: field.TypeInt, Default: 0},
		{Name: "external_id", Type: field.TypeString, Nullable: true},
		{Name: "licenses_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1AccountsTable holds the schema information for the "s1_accounts" table.
	S1AccountsTable = &schema.Table{
		Name:       "s1_accounts",
		Columns:    S1AccountsColumns,
		PrimaryKey: []*schema.Column{S1AccountsColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1account_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1AccountsColumns[1]},
			},
			{
				Name:    "bronzes1account_state",
				Unique:  false,
				Columns: []*schema.Column{S1AccountsColumns[4]},
			},
			{
				Name:    "bronzes1account_account_type",
				Unique:  false,
				Columns: []*schema.Column{S1AccountsColumns[5]},
			},
		},
	}
	// S1AgentsColumns holds the columns for the "s1_agents" table.
	S1AgentsColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "computer_name", Type: field.TypeString},
		{Name: "external_ip", Type: field.TypeString, Nullable: true},
		{Name: "site_name", Type: field.TypeString, Nullable: true},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "account_name", Type: field.TypeString, Nullable: true},
		{Name: "agent_version", Type: field.TypeString, Nullable: true},
		{Name: "os_type", Type: field.TypeString, Nullable: true},
		{Name: "os_name", Type: field.TypeString, Nullable: true},
		{Name: "os_revision", Type: field.TypeString, Nullable: true},
		{Name: "os_arch", Type: field.TypeString, Nullable: true},
		{Name: "is_active", Type: field.TypeBool, Default: false},
		{Name: "is_infected", Type: field.TypeBool, Default: false},
		{Name: "is_decommissioned", Type: field.TypeBool, Default: false},
		{Name: "machine_type", Type: field.TypeString, Nullable: true},
		{Name: "domain", Type: field.TypeString, Nullable: true},
		{Name: "uuid", Type: field.TypeString, Nullable: true},
		{Name: "network_status", Type: field.TypeString, Nullable: true},
		{Name: "last_active_date", Type: field.TypeTime, Nullable: true},
		{Name: "registered_at", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "os_start_time", Type: field.TypeTime, Nullable: true},
		{Name: "active_threats", Type: field.TypeInt, Default: 0},
		{Name: "encrypted_applications", Type: field.TypeBool, Default: false},
		{Name: "group_name", Type: field.TypeString, Nullable: true},
		{Name: "group_id", Type: field.TypeString, Nullable: true},
		{Name: "cpu_count", Type: field.TypeInt, Default: 0},
		{Name: "core_count", Type: field.TypeInt, Default: 0},
		{Name: "cpu_id", Type: field.TypeString, Nullable: true},
		{Name: "total_memory", Type: field.TypeInt64, Default: 0},
		{Name: "model_name", Type: field.TypeString, Nullable: true},
		{Name: "serial_number", Type: field.TypeString, Nullable: true},
		{Name: "storage_encryption_status", Type: field.TypeString, Nullable: true},
		{Name: "network_interfaces_json", Type: field.TypeJSON, Nullable: true},
		{Name: "site_id", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "os_username", Type: field.TypeString, Nullable: true},
		{Name: "group_ip", Type: field.TypeString, Nullable: true},
		{Name: "scan_status", Type: field.TypeString, Nullable: true},
		{Name: "scan_started_at", Type: field.TypeTime, Nullable: true},
		{Name: "scan_finished_at", Type: field.TypeTime, Nullable: true},
		{Name: "mitigation_mode", Type: field.TypeString, Nullable: true},
		{Name: "mitigation_mode_suspicious", Type: field.TypeString, Nullable: true},
		{Name: "last_logged_in_user_name", Type: field.TypeString, Nullable: true},
		{Name: "installer_type", Type: field.TypeString, Nullable: true},
		{Name: "external_id", Type: field.TypeString, Nullable: true},
		{Name: "last_ip_to_mgmt", Type: field.TypeString, Nullable: true},
		{Name: "is_up_to_date", Type: field.TypeBool, Default: false},
		{Name: "is_pending_uninstall", Type: field.TypeBool, Default: false},
		{Name: "is_uninstalled", Type: field.TypeBool, Default: false},
		{Name: "apps_vulnerability_status", Type: field.TypeString, Nullable: true},
		{Name: "console_migration_status", Type: field.TypeString, Nullable: true},
		{Name: "ranger_version", Type: field.TypeString, Nullable: true},
		{Name: "ranger_status", Type: field.TypeString, Nullable: true},
		{Name: "active_directory_json", Type: field.TypeJSON, Nullable: true},
		{Name: "locations_json", Type: field.TypeJSON, Nullable: true},
		{Name: "user_actions_needed_json", Type: field.TypeJSON, Nullable: true},
		{Name: "missing_permissions_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1AgentsTable holds the schema information for the "s1_agents" table.
	S1AgentsTable = &schema.Table{
		Name:       "s1_agents",
		Columns:    S1AgentsColumns,
		PrimaryKey: []*schema.Column{S1AgentsColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1agent_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsColumns[6]},
			},
			{
				Name:    "bronzes1agent_is_active",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsColumns[13]},
			},
			{
				Name:    "bronzes1agent_is_infected",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsColumns[14]},
			},
			{
				Name:    "bronzes1agent_os_type",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsColumns[9]},
			},
			{
				Name:    "bronzes1agent_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsColumns[1]},
			},
			{
				Name:    "bronzes1agent_site_id",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsColumns[36]},
			},
			{
				Name:    "bronzes1agent_scan_status",
				Unique:  false,
				Columns: []*schema.Column{S1AgentsColumns[40]},
			},
		},
	}
	// S1AgentNicsColumns holds the columns for the "s1_agent_nics" table.
	S1AgentNicsColumns = []*schema.Column{
		{Name: "id", Type: field.TypeInt, Increment: true},
		{Name: "interface_id", Type: field.TypeString, Nullable: true},
		{Name: "name", Type: field.TypeString, Nullable: true},
		{Name: "description", Type: field.TypeString, Nullable: true},
		{Name: "type", Type: field.TypeString, Nullable: true},
		{Name: "inet_json", Type: field.TypeJSON, Nullable: true},
		{Name: "inet6_json", Type: field.TypeJSON, Nullable: true},
		{Name: "physical", Type: field.TypeString, Nullable: true},
		{Name: "gateway_ip", Type: field.TypeString, Nullable: true},
		{Name: "gateway_mac", Type: field.TypeString, Nullable: true},
		{Name: "bronze_s1agent_nics", Type: field.TypeString},
	}
	// S1AgentNicsTable holds the schema information for the "s1_agent_nics" table.
	S1AgentNicsTable = &schema.Table{
		Name:       "s1_agent_nics",
		Columns:    S1AgentNicsColumns,
		PrimaryKey: []*schema.Column{S1AgentNicsColumns[0]},
		ForeignKeys: []*schema.ForeignKey{
			{
				Symbol:     "s1_agent_nics_s1_agents_nics",
				Columns:    []*schema.Column{S1AgentNicsColumns[10]},
				RefColumns: []*schema.Column{S1AgentsColumns[0]},
				OnDelete:   schema.NoAction,
			},
		},
	}
	// S1AppInventoryColumns holds the columns for the "s1_app_inventory" table.
	S1AppInventoryColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "application_name", Type: field.TypeString},
		{Name: "application_vendor", Type: field.TypeString, Nullable: true},
		{Name: "endpoints_count", Type: field.TypeInt, Nullable: true},
		{Name: "application_versions_count", Type: field.TypeInt, Nullable: true},
		{Name: "estimate", Type: field.TypeBool, Default: false},
	}
	// S1AppInventoryTable holds the schema information for the "s1_app_inventory" table.
	S1AppInventoryTable = &schema.Table{
		Name:       "s1_app_inventory",
		Columns:    S1AppInventoryColumns,
		PrimaryKey: []*schema.Column{S1AppInventoryColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1appinventory_application_name",
				Unique:  false,
				Columns: []*schema.Column{S1AppInventoryColumns[3]},
			},
			{
				Name:    "bronzes1appinventory_application_vendor",
				Unique:  false,
				Columns: []*schema.Column{S1AppInventoryColumns[4]},
			},
			{
				Name:    "bronzes1appinventory_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1AppInventoryColumns[1]},
			},
		},
	}
	// S1EndpointAppsColumns holds the columns for the "s1_endpoint_apps" table.
	S1EndpointAppsColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "agent_id", Type: field.TypeString},
		{Name: "name", Type: field.TypeString},
		{Name: "version", Type: field.TypeString, Nullable: true},
		{Name: "publisher", Type: field.TypeString, Nullable: true},
		{Name: "size", Type: field.TypeInt, Nullable: true},
		{Name: "installed_date", Type: field.TypeTime, Nullable: true},
	}
	// S1EndpointAppsTable holds the schema information for the "s1_endpoint_apps" table.
	S1EndpointAppsTable = &schema.Table{
		Name:       "s1_endpoint_apps",
		Columns:    S1EndpointAppsColumns,
		PrimaryKey: []*schema.Column{S1EndpointAppsColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1endpointapp_agent_id",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsColumns[3]},
			},
			{
				Name:    "bronzes1endpointapp_name",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsColumns[4]},
			},
			{
				Name:    "bronzes1endpointapp_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsColumns[1]},
			},
			{
				Name:    "bronzes1endpointapp_name_version",
				Unique:  false,
				Columns: []*schema.Column{S1EndpointAppsColumns[4], S1EndpointAppsColumns[5]},
			},
		},
	}
	// S1GroupsColumns holds the columns for the "s1_groups" table.
	S1GroupsColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "name", Type: field.TypeString},
		{Name: "site_id", Type: field.TypeString, Nullable: true},
		{Name: "type", Type: field.TypeString, Nullable: true},
		{Name: "is_default", Type: field.TypeBool, Default: false},
		{Name: "inherits", Type: field.TypeBool, Default: false},
		{Name: "rank", Type: field.TypeInt, Nullable: true},
		{Name: "total_agents", Type: field.TypeInt, Default: 0},
		{Name: "creator", Type: field.TypeString, Nullable: true},
		{Name: "creator_id", Type: field.TypeString, Nullable: true},
		{Name: "filter_name", Type: field.TypeString, Nullable: true},
		{Name: "filter_id", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "registration_token", Type: field.TypeString, Nullable: true},
	}
	// S1GroupsTable holds the schema information for the "s1_groups" table.
	S1GroupsTable = &schema.Table{
		Name:       "s1_groups",
		Columns:    S1GroupsColumns,
		PrimaryKey: []*schema.Column{S1GroupsColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1group_site_id",
				Unique:  false,
				Columns: []*schema.Column{S1GroupsColumns[4]},
			},
			{
				Name:    "bronzes1group_type",
				Unique:  false,
				Columns: []*schema.Column{S1GroupsColumns[5]},
			},
			{
				Name:    "bronzes1group_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1GroupsColumns[1]},
			},
		},
	}
	// S1NetworkDiscoveriesColumns holds the columns for the "s1_network_discoveries" table.
	S1NetworkDiscoveriesColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "name", Type: field.TypeString, Nullable: true},
		{Name: "ip_address", Type: field.TypeString, Nullable: true},
		{Name: "domain", Type: field.TypeString, Nullable: true},
		{Name: "serial_number", Type: field.TypeString, Nullable: true},
		{Name: "category", Type: field.TypeString, Nullable: true},
		{Name: "sub_category", Type: field.TypeString, Nullable: true},
		{Name: "resource_type", Type: field.TypeString, Nullable: true},
		{Name: "os", Type: field.TypeString, Nullable: true},
		{Name: "os_family", Type: field.TypeString, Nullable: true},
		{Name: "os_version", Type: field.TypeString, Nullable: true},
		{Name: "os_name_version", Type: field.TypeString, Nullable: true},
		{Name: "architecture", Type: field.TypeString, Nullable: true},
		{Name: "manufacturer", Type: field.TypeString, Nullable: true},
		{Name: "cpu", Type: field.TypeString, Nullable: true},
		{Name: "memory_readable", Type: field.TypeString, Nullable: true},
		{Name: "network_name", Type: field.TypeString, Nullable: true},
		{Name: "asset_status", Type: field.TypeString, Nullable: true},
		{Name: "asset_criticality", Type: field.TypeString, Nullable: true},
		{Name: "asset_environment", Type: field.TypeString, Nullable: true},
		{Name: "infection_status", Type: field.TypeString, Nullable: true},
		{Name: "device_review", Type: field.TypeString, Nullable: true},
		{Name: "epp_unsupported_unknown", Type: field.TypeString, Nullable: true},
		{Name: "asset_contact_email", Type: field.TypeString, Nullable: true},
		{Name: "legacy_identity_policy_name", Type: field.TypeString, Nullable: true},
		{Name: "previous_os_type", Type: field.TypeString, Nullable: true},
		{Name: "previous_os_version", Type: field.TypeString, Nullable: true},
		{Name: "previous_device_function", Type: field.TypeString, Nullable: true},
		{Name: "detected_from_site", Type: field.TypeString, Nullable: true},
		{Name: "s1_account_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_account_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_site_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_site_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_group_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_group_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_scope_id", Type: field.TypeString, Nullable: true},
		{Name: "s1_scope_level", Type: field.TypeString, Nullable: true},
		{Name: "s1_scope_path", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_account_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_group_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_site_name", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_scope_level", Type: field.TypeString, Nullable: true},
		{Name: "s1_onboarded_scope_path", Type: field.TypeString, Nullable: true},
		{Name: "memory", Type: field.TypeInt, Nullable: true},
		{Name: "core_count", Type: field.TypeInt, Nullable: true},
		{Name: "s1_management_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_scope_type", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_account_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_group_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_scope_id", Type: field.TypeInt, Nullable: true},
		{Name: "s1_onboarded_site_id", Type: field.TypeInt, Nullable: true},
		{Name: "is_ad_connector", Type: field.TypeBool, Default: false},
		{Name: "is_dc_server", Type: field.TypeBool, Default: false},
		{Name: "ads_enabled", Type: field.TypeBool, Default: false},
		{Name: "first_seen_dt", Type: field.TypeTime, Nullable: true},
		{Name: "last_update_dt", Type: field.TypeTime, Nullable: true},
		{Name: "last_active_dt", Type: field.TypeTime, Nullable: true},
		{Name: "last_reboot_dt", Type: field.TypeTime, Nullable: true},
		{Name: "s1_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "agent_json", Type: field.TypeJSON, Nullable: true},
		{Name: "network_interfaces_json", Type: field.TypeJSON, Nullable: true},
		{Name: "alerts_json", Type: field.TypeJSON, Nullable: true},
		{Name: "alerts_count_json", Type: field.TypeJSON, Nullable: true},
		{Name: "device_review_log_json", Type: field.TypeJSON, Nullable: true},
		{Name: "identity_json", Type: field.TypeJSON, Nullable: true},
		{Name: "notes_json", Type: field.TypeJSON, Nullable: true},
		{Name: "tags_json", Type: field.TypeJSON, Nullable: true},
		{Name: "missing_coverage_json", Type: field.TypeJSON, Nullable: true},
		{Name: "subnets_json", Type: field.TypeJSON, Nullable: true},
		{Name: "surfaces_json", Type: field.TypeJSON, Nullable: true},
		{Name: "network_names_json", Type: field.TypeJSON, Nullable: true},
		{Name: "risk_factors_json", Type: field.TypeJSON, Nullable: true},
		{Name: "active_coverage_json", Type: field.TypeJSON, Nullable: true},
		{Name: "discovery_methods_json", Type: field.TypeJSON, Nullable: true},
		{Name: "hostnames_json", Type: field.TypeJSON, Nullable: true},
		{Name: "internal_ips_json", Type: field.TypeJSON, Nullable: true},
		{Name: "internal_ips_v6_json", Type: field.TypeJSON, Nullable: true},
		{Name: "mac_addresses_json", Type: field.TypeJSON, Nullable: true},
		{Name: "gateway_ips_json", Type: field.TypeJSON, Nullable: true},
		{Name: "gateway_macs_json", Type: field.TypeJSON, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "ranger_tags_json", Type: field.TypeJSON, Nullable: true},
		{Name: "id_secondary_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1NetworkDiscoveriesTable holds the schema information for the "s1_network_discoveries" table.
	S1NetworkDiscoveriesTable = &schema.Table{
		Name:       "s1_network_discoveries",
		Columns:    S1NetworkDiscoveriesColumns,
		PrimaryKey: []*schema.Column{S1NetworkDiscoveriesColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1networkdiscovery_asset_status",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesColumns[19]},
			},
			{
				Name:    "bronzes1networkdiscovery_s1_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesColumns[31]},
			},
			{
				Name:    "bronzes1networkdiscovery_s1_site_id",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesColumns[33]},
			},
			{
				Name:    "bronzes1networkdiscovery_infection_status",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesColumns[22]},
			},
			{
				Name:    "bronzes1networkdiscovery_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesColumns[1]},
			},
			{
				Name:    "bronzes1networkdiscovery_device_review",
				Unique:  false,
				Columns: []*schema.Column{S1NetworkDiscoveriesColumns[23]},
			},
		},
	}
	// S1RangerDevicesColumns holds the columns for the "s1_ranger_devices" table.
	S1RangerDevicesColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "local_ip", Type: field.TypeString, Nullable: true},
		{Name: "external_ip", Type: field.TypeString, Nullable: true},
		{Name: "mac_address", Type: field.TypeString, Nullable: true},
		{Name: "os_type", Type: field.TypeString, Nullable: true},
		{Name: "os_name", Type: field.TypeString, Nullable: true},
		{Name: "os_version", Type: field.TypeString, Nullable: true},
		{Name: "device_type", Type: field.TypeString, Nullable: true},
		{Name: "device_function", Type: field.TypeString, Nullable: true},
		{Name: "manufacturer", Type: field.TypeString, Nullable: true},
		{Name: "managed_state", Type: field.TypeString, Nullable: true},
		{Name: "agent_id", Type: field.TypeString, Nullable: true},
		{Name: "first_seen", Type: field.TypeTime, Nullable: true},
		{Name: "last_seen", Type: field.TypeTime, Nullable: true},
		{Name: "subnet_address", Type: field.TypeString, Nullable: true},
		{Name: "gateway_ip_address", Type: field.TypeString, Nullable: true},
		{Name: "gateway_mac_address", Type: field.TypeString, Nullable: true},
		{Name: "network_name", Type: field.TypeString, Nullable: true},
		{Name: "domain", Type: field.TypeString, Nullable: true},
		{Name: "site_name", Type: field.TypeString, Nullable: true},
		{Name: "device_review", Type: field.TypeString, Nullable: true},
		{Name: "has_identity", Type: field.TypeBool, Default: false},
		{Name: "has_user_label", Type: field.TypeBool, Default: false},
		{Name: "fingerprint_score", Type: field.TypeInt, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "hostnames_json", Type: field.TypeJSON, Nullable: true},
		{Name: "discovery_methods_json", Type: field.TypeJSON, Nullable: true},
		{Name: "networks_json", Type: field.TypeJSON, Nullable: true},
		{Name: "tags_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1RangerDevicesTable holds the schema information for the "s1_ranger_devices" table.
	S1RangerDevicesTable = &schema.Table{
		Name:       "s1_ranger_devices",
		Columns:    S1RangerDevicesColumns,
		PrimaryKey: []*schema.Column{S1RangerDevicesColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1rangerdevice_managed_state",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesColumns[12]},
			},
			{
				Name:    "bronzes1rangerdevice_agent_id",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesColumns[13]},
			},
			{
				Name:    "bronzes1rangerdevice_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesColumns[1]},
			},
			{
				Name:    "bronzes1rangerdevice_site_name",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesColumns[21]},
			},
			{
				Name:    "bronzes1rangerdevice_network_name",
				Unique:  false,
				Columns: []*schema.Column{S1RangerDevicesColumns[19]},
			},
		},
	}
	// S1RangerGatewaysColumns holds the columns for the "s1_ranger_gateways" table.
	S1RangerGatewaysColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "ip", Type: field.TypeString, Nullable: true},
		{Name: "mac_address", Type: field.TypeString, Nullable: true},
		{Name: "external_ip", Type: field.TypeString, Nullable: true},
		{Name: "manufacturer", Type: field.TypeString, Nullable: true},
		{Name: "network_name", Type: field.TypeString, Nullable: true},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "account_name", Type: field.TypeString, Nullable: true},
		{Name: "site_id", Type: field.TypeString, Nullable: true},
		{Name: "number_of_agents", Type: field.TypeInt, Nullable: true},
		{Name: "number_of_rangers", Type: field.TypeInt, Nullable: true},
		{Name: "connected_rangers", Type: field.TypeInt, Nullable: true},
		{Name: "total_agents", Type: field.TypeInt, Nullable: true},
		{Name: "agent_percentage", Type: field.TypeFloat64, Nullable: true},
		{Name: "allow_scan", Type: field.TypeBool, Default: false},
		{Name: "archived", Type: field.TypeBool, Default: false},
		{Name: "new_network", Type: field.TypeBool, Default: false},
		{Name: "inherit_settings", Type: field.TypeBool, Default: false},
		{Name: "tcp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "udp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "icmp_scan", Type: field.TypeBool, Default: false},
		{Name: "smb_scan", Type: field.TypeBool, Default: false},
		{Name: "mdns_scan", Type: field.TypeBool, Default: false},
		{Name: "rdns_scan", Type: field.TypeBool, Default: false},
		{Name: "snmp_scan", Type: field.TypeBool, Default: false},
		{Name: "scan_only_local_subnets", Type: field.TypeBool, Default: false},
		{Name: "created_at_api", Type: field.TypeTime, Nullable: true},
		{Name: "expiry_date", Type: field.TypeTime, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "restrictions_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1RangerGatewaysTable holds the schema information for the "s1_ranger_gateways" table.
	S1RangerGatewaysTable = &schema.Table{
		Name:       "s1_ranger_gateways",
		Columns:    S1RangerGatewaysColumns,
		PrimaryKey: []*schema.Column{S1RangerGatewaysColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1rangergateway_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysColumns[8]},
			},
			{
				Name:    "bronzes1rangergateway_site_id",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysColumns[10]},
			},
			{
				Name:    "bronzes1rangergateway_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysColumns[1]},
			},
			{
				Name:    "bronzes1rangergateway_network_name",
				Unique:  false,
				Columns: []*schema.Column{S1RangerGatewaysColumns[7]},
			},
		},
	}
	// S1RangerSettingsColumns holds the columns for the "s1_ranger_settings" table.
	S1RangerSettingsColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "scope_id", Type: field.TypeString, Nullable: true},
		{Name: "enabled", Type: field.TypeBool, Default: false},
		{Name: "use_periodic_snapshots", Type: field.TypeBool, Default: false},
		{Name: "snapshot_period", Type: field.TypeInt, Nullable: true},
		{Name: "network_decommission_value", Type: field.TypeInt, Nullable: true},
		{Name: "min_agents_in_network_to_scan", Type: field.TypeInt, Nullable: true},
		{Name: "tcp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "udp_port_scan", Type: field.TypeBool, Default: false},
		{Name: "icmp_scan", Type: field.TypeBool, Default: false},
		{Name: "smb_scan", Type: field.TypeBool, Default: false},
		{Name: "mdns_scan", Type: field.TypeBool, Default: false},
		{Name: "rdns_scan", Type: field.TypeBool, Default: false},
		{Name: "snmp_scan", Type: field.TypeBool, Default: false},
		{Name: "multi_scan_ssdp", Type: field.TypeBool, Default: false},
		{Name: "use_full_dns_scan", Type: field.TypeBool, Default: false},
		{Name: "scan_only_local_subnets", Type: field.TypeBool, Default: false},
		{Name: "auto_enable_networks", Type: field.TypeBool, Default: false},
		{Name: "combine_devices", Type: field.TypeBool, Default: false},
		{Name: "new_network_in_hours", Type: field.TypeInt, Nullable: true},
		{Name: "tcp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "udp_ports_json", Type: field.TypeJSON, Nullable: true},
		{Name: "restrictions_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1RangerSettingsTable holds the schema information for the "s1_ranger_settings" table.
	S1RangerSettingsTable = &schema.Table{
		Name:       "s1_ranger_settings",
		Columns:    S1RangerSettingsColumns,
		PrimaryKey: []*schema.Column{S1RangerSettingsColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1rangersetting_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1RangerSettingsColumns[3]},
			},
			{
				Name:    "bronzes1rangersetting_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1RangerSettingsColumns[1]},
			},
		},
	}
	// S1SitesColumns holds the columns for the "s1_sites" table.
	S1SitesColumns = []*schema.Column{
		{Name: "resource_id", Type: field.TypeString, Unique: true},
		{Name: "collected_at", Type: field.TypeTime},
		{Name: "first_collected_at", Type: field.TypeTime},
		{Name: "name", Type: field.TypeString},
		{Name: "account_id", Type: field.TypeString, Nullable: true},
		{Name: "account_name", Type: field.TypeString, Nullable: true},
		{Name: "state", Type: field.TypeString, Nullable: true},
		{Name: "site_type", Type: field.TypeString, Nullable: true},
		{Name: "suite", Type: field.TypeString, Nullable: true},
		{Name: "creator", Type: field.TypeString, Nullable: true},
		{Name: "creator_id", Type: field.TypeString, Nullable: true},
		{Name: "health_status", Type: field.TypeBool, Default: false},
		{Name: "active_licenses", Type: field.TypeInt, Default: 0},
		{Name: "total_licenses", Type: field.TypeInt, Default: 0},
		{Name: "unlimited_licenses", Type: field.TypeBool, Default: false},
		{Name: "is_default", Type: field.TypeBool, Default: false},
		{Name: "description", Type: field.TypeString, Nullable: true},
		{Name: "api_created_at", Type: field.TypeTime, Nullable: true},
		{Name: "expiration", Type: field.TypeTime, Nullable: true},
		{Name: "api_updated_at", Type: field.TypeTime, Nullable: true},
		{Name: "external_id", Type: field.TypeString, Nullable: true},
		{Name: "sku", Type: field.TypeString, Nullable: true},
		{Name: "usage_type", Type: field.TypeString, Nullable: true},
		{Name: "unlimited_expiration", Type: field.TypeBool, Default: false},
		{Name: "inherit_account_expiration", Type: field.TypeBool, Default: false},
		{Name: "licenses_json", Type: field.TypeJSON, Nullable: true},
	}
	// S1SitesTable holds the schema information for the "s1_sites" table.
	S1SitesTable = &schema.Table{
		Name:       "s1_sites",
		Columns:    S1SitesColumns,
		PrimaryKey: []*schema.Column{S1SitesColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "bronzes1site_account_id",
				Unique:  false,
				Columns: []*schema.Column{S1SitesColumns[4]},
			},
			{
				Name:    "bronzes1site_state",
				Unique:  false,
				Columns: []*schema.Column{S1SitesColumns[6]},
			},
			{
				Name:    "bronzes1site_collected_at",
				Unique:  false,
				Columns: []*schema.Column{S1SitesColumns[1]},
			},
		},
	}
	// Tables holds all the tables in the schema.
	Tables = []*schema.Table{
		S1AccountsHistoryTable,
		S1AgentsHistoryTable,
		S1AgentNicsHistoryTable,
		S1AppInventoryHistoryTable,
		S1EndpointAppsHistoryTable,
		S1GroupsHistoryTable,
		S1NetworkDiscoveriesHistoryTable,
		S1RangerDevicesHistoryTable,
		S1RangerGatewaysHistoryTable,
		S1RangerSettingsHistoryTable,
		S1SitesHistoryTable,
		S1AccountsTable,
		S1AgentsTable,
		S1AgentNicsTable,
		S1AppInventoryTable,
		S1EndpointAppsTable,
		S1GroupsTable,
		S1NetworkDiscoveriesTable,
		S1RangerDevicesTable,
		S1RangerGatewaysTable,
		S1RangerSettingsTable,
		S1SitesTable,
	}
)

func init() {
	S1AccountsHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_accounts_history",
	}
	S1AgentsHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_agents_history",
	}
	S1AgentNicsHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_agent_nics_history",
	}
	S1AppInventoryHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_app_inventory_history",
	}
	S1EndpointAppsHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_endpoint_apps_history",
	}
	S1GroupsHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_groups_history",
	}
	S1NetworkDiscoveriesHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_network_discoveries_history",
	}
	S1RangerDevicesHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_ranger_devices_history",
	}
	S1RangerGatewaysHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_ranger_gateways_history",
	}
	S1RangerSettingsHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_ranger_settings_history",
	}
	S1SitesHistoryTable.Annotation = &entsql.Annotation{
		Table: "s1_sites_history",
	}
	S1AccountsTable.Annotation = &entsql.Annotation{
		Table: "s1_accounts",
	}
	S1AgentsTable.Annotation = &entsql.Annotation{
		Table: "s1_agents",
	}
	S1AgentNicsTable.ForeignKeys[0].RefTable = S1AgentsTable
	S1AgentNicsTable.Annotation = &entsql.Annotation{
		Table: "s1_agent_nics",
	}
	S1AppInventoryTable.Annotation = &entsql.Annotation{
		Table: "s1_app_inventory",
	}
	S1EndpointAppsTable.Annotation = &entsql.Annotation{
		Table: "s1_endpoint_apps",
	}
	S1GroupsTable.Annotation = &entsql.Annotation{
		Table: "s1_groups",
	}
	S1NetworkDiscoveriesTable.Annotation = &entsql.Annotation{
		Table: "s1_network_discoveries",
	}
	S1RangerDevicesTable.Annotation = &entsql.Annotation{
		Table: "s1_ranger_devices",
	}
	S1RangerGatewaysTable.Annotation = &entsql.Annotation{
		Table: "s1_ranger_gateways",
	}
	S1RangerSettingsTable.Annotation = &entsql.Annotation{
		Table: "s1_ranger_settings",
	}
	S1SitesTable.Annotation = &entsql.Annotation{
		Table: "s1_sites",
	}
}
