...
func NewActivities(configService *config.Service, entClient *entdataproc.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestDataprocClusters(ctx context.Context, params IngestDataprocClustersParams) (*IngestDataprocClustersResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest Dataproc clusters: %w", err))
	}
