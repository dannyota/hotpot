...
func NewActivities(configService *config.Service, entClient *entsecretmanager.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestSecretManagerSecrets(ctx context.Context, params IngestSecretManagerSecretsParams) (*IngestSecretManagerSecretsResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest secrets: %w", err))
	}
