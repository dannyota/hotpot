...
func NewHistoryService(entClient *entbigtable.Client) *HistoryService {
	return &HistoryService{entClient: entClient}
}

// CreateHistory creates initial history records for a new Bigtable instance.
func (h *HistoryService) CreateHistory(ctx context.Context, tx *entbigtable.Tx, data *InstanceData, now time.Time) error {
...
func (h *HistoryService) UpdateHistory(ctx context.Context, tx *entbigtable.Tx, old *entbigtable.BronzeGCPBigtableInstance, new *InstanceData, diff *InstanceDiff, now time.Time) error {
...
func (h *HistoryService) CloseHistory(ctx context.Context, tx *entbigtable.Tx, resourceID string, now time.Time) error {
	currentHistory, err := tx.BronzeHistoryGCPBigtableInstance.Query().
		Where(
