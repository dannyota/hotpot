...
func NewActivities(configService *config.Service, entClient *entserviceusage.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestEnabledServices(ctx context.Context, params IngestEnabledServicesParams) (*IngestEnabledServicesResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest enabled services: %w", err))
	}
