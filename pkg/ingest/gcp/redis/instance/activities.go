...
func NewActivities(configService *config.Service, entClient *entredis.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestRedisInstances(ctx context.Context, params IngestRedisInstancesParams) (*IngestRedisInstancesResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest Redis instances: %w", err))
	}
