...
func NewActivities(configService *config.Service, entClient *entappengine.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestAppEngineApplications(ctx context.Context, params IngestAppEngineApplicationsParams) (*IngestAppEngineApplicationsResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest App Engine applications: %w", err))
	}
