...
func NewActivities(configService *config.Service, entClient *entiap.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestIAPIAMPolicy(ctx context.Context, params IngestIAPIAMPolicyParams) (*IngestIAPIAMPolicyResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest IAP IAM policy: %w", err))
	}
