...
func NewActivities(configService *config.Service, entClient *entcloudfunctions.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestCloudFunctionsFunctions(ctx context.Context, params IngestCloudFunctionsFunctionsParams) (*IngestCloudFunctionsFunctionsResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest Cloud Functions: %w", err))
	}
