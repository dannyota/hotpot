...
func NewActivities(configService *config.Service, entClient *entrun.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestRunRevisions(ctx context.Context, params IngestRunRevisionsParams) (*IngestRunRevisionsResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest Cloud Run revisions: %w", err))
	}
