...
func (h *HistoryService) CreateHistory(ctx context.Context, tx *entcompute.Tx, ruleData *ForwardingRuleData, now time.Time) error {
	...
	for _, label := range ruleData.Labels {
		_, err := tx.BronzeHistoryGCPComputeForwardingRuleLabel.Create().
			SetForwardingRuleHistoryID(ruleHistory.ID).
			SetValidFrom(now).
			SetKey(label.Key).
...
func (h *HistoryService) UpdateHistory(ctx context.Context, tx *entcompute.Tx, old *entcompute.BronzeGCPComputeForwardingRule, new *ForwardingRuleData, diff *ForwardingRuleDiff, now time.Time) error {
	...
	if diff.IsChanged {
		...
		_, err := tx.BronzeHistoryGCPComputeForwardingRuleLabel.Update().
			Where(
				bronzehistorygcpcomputeforwardingrulelabel.ForwardingRuleHistoryID(currentHistory.ID),
		...
		for _, label := range new.Labels {
			_, err := tx.BronzeHistoryGCPComputeForwardingRuleLabel.Create().
				SetForwardingRuleHistoryID(newHistory.ID).
				SetValidFrom(now).
				SetKey(label.Key).
				SetValue(label.Value).
				Save(ctx)
		...
		for _, label := range new.Labels {
			_, err := tx.BronzeHistoryGCPComputeForwardingRuleLabel.Create().
				SetForwardingRuleHistoryID(currentHistory.ID).
				SetValidFrom(now).
				SetKey(label.Key).
				SetValue(label.Value).
				Save(ctx)
