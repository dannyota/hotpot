...
// HistoryService handles history tracking for forwarding rules.
...
// CreateHistory creates history records for a new forwarding rule and all children.
func (h *HistoryService) CreateHistory(tx *gorm.DB, fr *bronze.GCPComputeForwardingRule, now time.Time) error {
	// Create forwarding rule history
	frHist := toForwardingRuleHistory(fr, now)
	if err := tx.Create(&frHist).Error; err != nil {
		return err
	}

	// Create children history with forwarding_rule_history_id
	return h.createChildrenHistory(tx, frHist.HistoryID, fr, now)
}

// UpdateHistory closes old history and creates new history based on diff.
func (h *HistoryService) UpdateHistory(tx *gorm.DB, old, new *bronze.GCPComputeForwardingRule, diff *ForwardingRuleDiff, now time.Time) error {
	// Get current forwarding rule history
	var currentHist bronze_history.GCPComputeForwardingRule
	...
	// Forwarding rule unchanged, check children individually (granular tracking)
	return h.updateChildrenHistory(tx, currentHist.HistoryID, new, diff, now)
}

// CloseHistory closes history records for a deleted forwarding rule.
...
func (h *HistoryService) createChildrenHistory(tx *gorm.DB, forwardingRuleHistoryID uint, fr *bronze.GCPComputeForwardingRule, now time.Time) error {
	for _, label := range fr.Labels {
		labelHist := toLabelHistory(&label, forwardingRuleHistoryID, now)
...
func (h *HistoryService) closeChildrenHistory(tx *gorm.DB, forwardingRuleHistoryID uint, now time.Time) error {
	if err := tx.Table("bronze_history.gcp_compute_forwarding_rule_labels").
		Where("forwarding_rule_history_id = ? AND valid_to IS NULL", forwardingRuleHistoryID).
...
func (h *HistoryService) updateChildrenHistory(tx *gorm.DB, forwardingRuleHistoryID uint, new *bronze.GCPComputeForwardingRule, diff *ForwardingRuleDiff, now time.Time) error {
	if diff.LabelsDiff.Changed {
		if err := h.updateLabelsHistory(tx, forwardingRuleHistoryID, new.Labels, now); err != nil {
			return err
		}
	}
	return nil
...
func (h *HistoryService) updateLabelsHistory(tx *gorm.DB, forwardingRuleHistoryID uint, labels []bronze.GCPComputeForwardingRuleLabel, now time.Time) error {
	if err := tx.Table("bronze_history.gcp_compute_forwarding_rule_labels").
		Where("forwarding_rule_history_id = ? AND valid_to IS NULL", forwardingRuleHistoryID).
		Update("valid_to", now).Error; err != nil {
		return err
...
func toForwardingRuleHistory(fr *bronze.GCPComputeForwardingRule, now time.Time) bronze_history.GCPComputeForwardingRule {
	return bronze_history.GCPComputeForwardingRule{
...
func toLabelHistory(label *bronze.GCPComputeForwardingRuleLabel, forwardingRuleHistoryID uint, now time.Time) bronze_history.GCPComputeForwardingRuleLabel {
	return bronze_history.GCPComputeForwardingRuleLabel{
		ForwardingRuleHistoryID: forwardingRuleHistoryID,
		ValidFrom:               now,
		ValidTo:                 nil,
		Key:                     label.Key,
		Value:                   label.Value,
