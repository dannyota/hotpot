...
func (h *HistoryService) CreateHistory(ctx context.Context, tx *entcompute.Tx, data *UrlMapData, now time.Time) error {
...
func (h *HistoryService) UpdateHistory(ctx context.Context, tx *entcompute.Tx, old *entcompute.BronzeGCPComputeUrlMap, new *UrlMapData, diff *UrlMapDiff, now time.Time) error {
	if !diff.IsChanged {
		return nil
	}
