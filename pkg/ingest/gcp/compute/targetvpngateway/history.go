...
func (h *HistoryService) CreateHistory(ctx context.Context, tx *entvpn.Tx, data *TargetVpnGatewayData, now time.Time) error {
	...
	for _, label := range data.Labels {
		_, err := tx.BronzeHistoryGCPVPNTargetGatewayLabel.Create().
			SetTargetVpnGatewayHistoryID(gwHist.ID).
			SetValidFrom(now).
			SetKey(label.Key).
...
func (h *HistoryService) UpdateHistory(ctx context.Context, tx *entvpn.Tx, old *entvpn.BronzeGCPVPNTargetGateway, new *TargetVpnGatewayData, diff *TargetVpnGatewayDiff, now time.Time) error {
	...
	if diff.IsChanged {
		...
		_, err := tx.BronzeHistoryGCPVPNTargetGateway.UpdateOneID(currentHist.ID).
			SetValidTo(now).
			Save(ctx)
		if err != nil {
		...
		_, err = tx.BronzeHistoryGCPVPNTargetGatewayLabel.Update().
			Where(
				bronzehistorygcpvpntargetgatewaylabel.TargetVpnGatewayHistoryID(currentHist.ID),
	...
	if diff.LabelsDiff.HasChanges {
		...
		for _, label := range new.Labels {
			_, err := tx.BronzeHistoryGCPVPNTargetGatewayLabel.Create().
				SetTargetVpnGatewayHistoryID(currentHist.ID).
				SetValidFrom(now).
				SetKey(label.Key).
				SetValue(label.Value).
				Save(ctx)
