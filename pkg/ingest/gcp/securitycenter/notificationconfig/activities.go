...
func NewActivities(configService *config.Service, entClient *entsecuritycenter.Client, rmClient *entresourcemanager.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestNotificationConfigs(ctx context.Context, params IngestNotificationConfigsParams) (*IngestNotificationConfigsResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest SCC notification configs: %w", err))
	}
