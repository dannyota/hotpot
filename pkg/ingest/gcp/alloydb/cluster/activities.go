...
func NewActivities(configService *config.Service, entClient *entalloydb.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestAlloyDBClusters(ctx context.Context, params IngestAlloyDBClustersParams) (*IngestAlloyDBClustersResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest AlloyDB clusters: %w", err))
	}
