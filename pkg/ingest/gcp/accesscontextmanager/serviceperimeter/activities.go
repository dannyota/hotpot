...
func NewActivities(configService *config.Service, entClient *entaccesscontextmanager.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestServicePerimeters(ctx context.Context, params IngestServicePerimetersParams) (*IngestServicePerimetersResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest service perimeters: %w", err))
	}
