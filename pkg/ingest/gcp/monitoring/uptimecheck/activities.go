...
func NewActivities(configService *config.Service, entClient *entmonitoring.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestUptimeChecks(ctx context.Context, params IngestUptimeChecksParams) (*IngestUptimeChecksResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest uptime check configs: %w", err))
	}
