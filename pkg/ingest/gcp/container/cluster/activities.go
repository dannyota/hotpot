...
func NewActivities(configService *config.Service, entClient *entcontainer.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestContainerClusters(ctx context.Context, params IngestContainerClustersParams) (*IngestContainerClustersResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest clusters: %w", err))
	}
