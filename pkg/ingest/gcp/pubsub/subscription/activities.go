...
func NewActivities(configService *config.Service, entClient *entpubsub.Client, limiter ratelimit.Limiter) *Activities {
	return &Activities{
...
func (a *Activities) IngestPubSubSubscriptions(ctx context.Context, params IngestPubSubSubscriptionsParams) (*IngestPubSubSubscriptionsResult, error) {
	...
	if err != nil {
		return nil, temporalerr.MaybeNonRetryable(fmt.Errorf("failed to ingest Pub/Sub subscriptions: %w", err))
	}
