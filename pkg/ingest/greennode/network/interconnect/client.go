...
// Client wraps the GreenNode SDK for interconnect operations.
type Client struct {
...
// ListInterconnects lists all interconnects, handling pagination.
func (c *Client) ListInterconnects(ctx context.Context) ([]*networkv2.Interconnect, error) {
	var all []*networkv2.Interconnect
	page := 1
	size := 50

	for {
		result, err := c.sdk.Network.ListInterconnects(ctx, &networkv2.ListInterconnectsRequest{
			Page: page,
			Size: size,
		})
		if err != nil {
			return nil, fmt.Errorf("list interconnects page %d: %w", page, err)
		}
