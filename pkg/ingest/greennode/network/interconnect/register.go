...
// Register registers interconnect workflows and activities with the Temporal worker.
func Register(w worker.Worker, configService *config.Service, entClient *entnet.Client, iamAuth *auth.IAMUserAuth, limiter ratelimit.Limiter) {
	...
	w.RegisterActivity(activities.IngestNetworkInterconnects)
	w.RegisterWorkflow(GreenNodeNetworkInterconnectWorkflow)
}
