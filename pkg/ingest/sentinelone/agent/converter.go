...
func ConvertNICs(nics []APINetworkInterface) []NICData {
	...
	for _, nic := range nics {
		...
		if len(nic.Inet) > 0 {
			data, err := json.Marshal(nic.Inet)
			if err != nil {
				slog.Warn("failed to marshal inet addresses", "nic", nic.ID, "error", err)
			} else {
				n.InetJSON = data
			}
		}
		if len(nic.Inet6) > 0 {
			data, err := json.Marshal(nic.Inet6)
			if err != nil {
				slog.Warn("failed to marshal inet6 addresses", "nic", nic.ID, "error", err)
			} else {
				n.Inet6JSON = data
			}
		}
