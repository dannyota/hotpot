...
func (s *Service) saveAccounts(ctx context.Context, accounts []*AccountData) error {
	...
	// Delete stale accounts not returned by the API.
	allDBIDs, err := tx.BronzeS1Account.Query().
		Select(bronzes1account.FieldID).
		Strings(ctx)
	if err != nil {
		tx.Rollback()
		return fmt.Errorf("query all account IDs: %w", err)
	}
	...
	for _, id := range allDBIDs {
		...
		if err := s.history.CloseHistory(ctx, tx, id, now); err != nil {
			tx.Rollback()
			return fmt.Errorf("close history for stale account %s: %w", id, err)
		}

		if err := tx.BronzeS1Account.DeleteOneID(id).Exec(ctx); err != nil {
			tx.Rollback()
			return fmt.Errorf("delete stale account %s: %w", id, err)
		}
		staleCount++
	}

	if staleCount > 0 {
		slog.Info("s1 accounts: deleted stale", "count", staleCount)
	}
