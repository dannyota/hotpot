...
type Activities struct {
	...
	entClient     *entinstalledsoftware.Client
	db            *sql.DB
	providers     map[string]Provider
}

// NewActivities creates an Activities instance.
func NewActivities(configService *config.Service, entClient *entinstalledsoftware.Client, db *sql.DB, providers []Provider) *Activities {
...
// NormalizeInstalledSoftwareProviderActivity function reference for Temporal registration.
var NormalizeInstalledSoftwareProviderActivity = (*Activities).NormalizeInstalledSoftwareProvider

// MergeInstalledSoftwareActivity function reference for Temporal registration.
var MergeInstalledSoftwareActivity = (*Activities).MergeInstalledSoftware
...
// NormalizeInstalledSoftwareProvider loads bronze data for one provider, bulk-upserts
// to installed_software_normalized, and deletes stale rows.
func (a *Activities) NormalizeInstalledSoftwareProvider(ctx context.Context, params NormalizeProviderParams) (*NormalizeProviderResult, error) {
	...
	for _, batch := range batches {
		sem <- struct{}{}
		go func(batch []NormalizedInstalledSoftware) {
			defer func() { <-sem }()
	...
	deleted, err := a.entClient.SilverInstalledSoftwareNormalized.Delete().
		Where(
			silverinstalledsoftwarenormalized.ProviderEQ(params.ProviderKey),
			silverinstalledsoftwarenormalized.NormalizedAtLT(now),
		).Exec(ctx)
	if err != nil {
...
func (a *Activities) upsertNormalizedBatch(ctx context.Context, batch []NormalizedInstalledSoftware, now time.Time) error {
	if len(batch) == 0 {
		return nil
	}

	var b strings.Builder
	b.WriteString(`INSERT INTO silver.installed_software_normalized
...
// MergeInstalledSoftwareResult holds merge statistics.
type MergeInstalledSoftwareResult struct {
	Created int
	Updated int
	Deleted int
}
...
// MergeInstalledSoftware reads all normalized rows, deduplicates by (machine_id, name)
// with S1 priority, and writes to silver.installed_software.
func (a *Activities) MergeInstalledSoftware(ctx context.Context) (*MergeInstalledSoftwareResult, error) {
	logger := activity.GetLogger(ctx)
	logger.Info("Starting installed software merge")

	// Read all normalized rows.
	normalizedRows, err := a.entClient.SilverInstalledSoftwareNormalized.Query().All(ctx)
	if err != nil {
	...
	rows := make([]NormalizedInstalledSoftware, 0, len(normalizedRows))
	for _, r := range normalizedRows {
		rows = append(rows, NormalizedInstalledSoftware{
			Provider:         r.Provider,
			IsBase:           r.IsBase,
	...
	existingRecords, err := a.entClient.SilverInstalledSoftware.Query().All(ctx)
	if err != nil {
	...
	deleted := 0
	if len(staleIDs) > 0 {
		_, err = a.entClient.SilverInstalledSoftwareBronzeLink.Delete().
			Where(silverinstalledsoftwarebronzelink.HasInstalledSoftwareWith(
				silverinstalledsoftware.IDIn(staleIDs...))).
			Exec(ctx)
		if err != nil {
			slog.Warn("Failed to delete stale bronze links", "count", len(staleIDs), "error", err)
		}
		deleted, err = a.entClient.SilverInstalledSoftware.Delete().
			Where(silverinstalledsoftware.IDIn(staleIDs...)).
			Exec(ctx)
		if err != nil {
	...
	return &MergeInstalledSoftwareResult{
		Created: created,
		Updated: updated,
		Deleted: deleted,
	}, nil
...
func (a *Activities) upsertMergedBatch(ctx context.Context, batch []*mergedEntry, now time.Time) error {
	...
	{
		var b strings.Builder
		b.WriteString(`INSERT INTO silver.installed_software
	...
	// 2. Delete old bronze_links for this batch.
	{
		...
		q := `DELETE FROM silver.installed_software_bronze_links
			WHERE silver_installed_software_bronze_links IN (` + strings.Join(placeholders, ",") + `)`
	...
	// 3. Bulk insert new bronze_links.
	{
		...
		if totalLinks > 0 {
			var b strings.Builder
			b.WriteString(`INSERT INTO silver.installed_software_bronze_links
				(provider, bronze_table, bronze_resource_id, silver_installed_software_bronze_links)
				VALUES `)
...
func makeBatches(records []NormalizedInstalledSoftware, size int) [][]NormalizedInstalledSoftware {
	var batches [][]NormalizedInstalledSoftware
