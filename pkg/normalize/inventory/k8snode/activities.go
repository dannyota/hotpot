...
import (
	...
	entk8snode "danny.vn/hotpot/pkg/storage/ent/k8snode"
...
func (a *Activities) NormalizeK8sNodeProvider(ctx context.Context, params NormalizeProviderParams) (*NormalizeProviderResult, error) {
	...
	for _, rec := range records {
		...
		if existingIDs[rid] {
			// Update existing row.
			_, err := a.entClient.InventoryK8sNodeNormalized.UpdateOneID(rid).
			...
			// Create new row.
			err := a.entClient.InventoryK8sNodeNormalized.Create().
				SetID(rid).
				SetProvider(rec.Provider).
				SetIsBase(rec.IsBase).
				SetBronzeTable(rec.BronzeTable).
...
// MergeK8sNodes reads all normalized rows, runs dedup, and writes to inventory.k8s_nodes.
func (a *Activities) MergeK8sNodes(ctx context.Context) (*MergeK8sNodesResult, error) {
	...
	for _, m := range merged {
		...
		if isNew {
			// Create new k8s node.
			err := a.entClient.InventoryK8sNode.Create().
				SetID(nodeID).
			...
			_, err := a.entClient.InventoryK8sNode.UpdateOneID(nodeID).
				SetNodeName(m.NodeName).
				SetClusterName(m.ClusterName).
				SetNodePool(m.NodePool).
				SetStatus(m.Status).
		...
		// Insert bronze links.
		for _, link := range m.BronzeLinks {
			err := a.entClient.InventoryK8sNodeBronzeLink.Create().
