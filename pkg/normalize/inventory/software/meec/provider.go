...
// Provider normalizes bronze.meec_inventory_installed_software into NormalizedSoftware records.
...
func (Provider) Load(ctx context.Context, db *sql.DB) ([]software.NormalizedSoftware, error) {
	rows, err := db.QueryContext(ctx, `
		...
		JOIN silver.inventory_machine_links l
			ON l.bronze_resource_id = c.resource_id AND l.bronze_table = 'meec_inventory_computers'
		JOIN silver.inventory_machines m ON m.resource_id = l.inventory_machine_bronze_links
	...
	for rows.Next() {
		...
		var installedDateMs sql.NullInt64
		if err := rows.Scan(&bronzeID, &machineID, &name, &version, &publisher,
			&collectedAt, &firstCollectedAt, &installedDateMs); err != nil {
			return nil, fmt.Errorf("scan meec installed software: %w", err)
		}
		var installedOn *time.Time
		if installedDateMs.Valid && installedDateMs.Int64 > 0 {
			t := time.UnixMilli(installedDateMs.Int64)
			installedOn = &t
		}
