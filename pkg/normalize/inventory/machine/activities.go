...
func (a *Activities) NormalizeMachineProvider(ctx context.Context, params NormalizeProviderParams) (*NormalizeProviderResult, error) {
	...
	for _, rec := range records {
		...
		if existingIDs[rid] {
			// Update existing row.
			_, err := a.entClient.InventoryMachineNormalized.UpdateOneID(rid).
			...
			// Create new row.
			err := a.entClient.InventoryMachineNormalized.Create().
				SetID(rid).
				SetProvider(rec.Provider).
				SetIsBase(rec.IsBase).
				SetBronzeTable(rec.BronzeTable).
...
// MergeMachines reads all normalized rows, runs dedup, and writes to inventory.machines.
func (a *Activities) MergeMachines(ctx context.Context) (*MergeMachinesResult, error) {
	...
	for _, m := range merged {
		...
		if isNew {
			// Create new machine.
			err := a.entClient.InventoryMachine.Create().
				SetID(machineID).
			...
			_, err := a.entClient.InventoryMachine.UpdateOneID(machineID).
				SetHostname(m.Hostname).
				SetOsType(m.OSType).
				SetOsName(m.OSName).
				SetStatus(m.Status).
		...
		// Insert bronze links.
		for _, link := range m.BronzeLinks {
			err := a.entClient.InventoryMachineBronzeLink.Create().
