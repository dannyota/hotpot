...
func (Provider) Load(ctx context.Context, db *sql.DB) ([]machine.NormalizedMachine, error) {
	...
	for rows.Next() {
		var resourceID, hostname, status, region, flavorName, imageVer, serverGroup, ifacesJSON, createdAtAPI string
		var collectedAt, firstCollectedAt sql.NullTime
		if err := rows.Scan(&resourceID, &hostname, &status, &region,
			&flavorName, &imageVer, &serverGroup, &collectedAt, &firstCollectedAt, &ifacesJSON, &createdAtAPI); err != nil {
		...
		if createdAtAPI != "" {
			if t, err := time.Parse(time.RFC3339, createdAtAPI); err == nil {
				created = &t
			}
		}
