...
func NewActivities(configService *config.Service, db *sql.DB) *Activities {
	return &Activities{
...
// loadRPMCoreRepos loads OS core RPM repo names from config.rpm_core_repos.
func (a *Activities) loadRPMCoreRepos(ctx context.Context) (map[string]bool, error) {
	rows, err := a.db.QueryContext(ctx, `
		SELECT repo_name FROM config.rpm_core_repos WHERE is_active = true`)
	if err != nil {
		return nil, fmt.Errorf("query rpm core repos: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name); err != nil {
			return nil, fmt.Errorf("scan rpm core repo: %w", err)
		}
		result[name] = true
	}
...
func (a *Activities) loadMatchRules(ctx context.Context) (*matchRulesData, error) {
	rows, err := a.db.QueryContext(ctx, `
		SELECT product_slug, rule_type, COALESCE(os_type, ''), value, extra_value
		FROM config.software_match_rules
...
func (a *Activities) loadOSCoreNames(ctx context.Context) (map[string]bool, error) {
	...
	// Load RPM core repos from config.
	rpmCoreRepos, err := a.loadRPMCoreRepos(ctx)
	if err != nil {
		return nil, fmt.Errorf("load rpm core repos: %w", err)
	}
...
func (a *Activities) loadOSCoreRules(ctx context.Context) (exact map[string]bool, prefixes []string, suffixes []string, err error) {
	rows, err := a.db.QueryContext(ctx, `
		SELECT rule_type, COALESCE(os_type, ''), value
		FROM config.os_core_rules
		WHERE is_active = true`)
	if err != nil {
